...
# Supply critical region as a list of bounds, ex: list(c(-Inf, -3), c(0, 1), c(3, Inf))
...
test_iter <- function(delta, test_stat, critical_region, mu_0, sigma, n, dist, dist_params, s){
...
test_sim <- function(N, delta, test_stat, critical_region, mu_0, sigma, n, dist, dist_params){
  output <- 1:N %>% 
    map(
      test_iter, 
      delta = delta, 
...
```{r}
# This function assumes dist = rnorm and dist_params = list(mu, 1)
power <- function(mu, N, delta, test_stat, critical_region, mu_0, sigma, n){
  dist <- rnorm
  dist_params <- list(mu, sigma)
  output <- test_sim(N, delta, test_stat, critical_region, mu_0, sigma, n, dist, dist_params) %>% 
    summarize(
